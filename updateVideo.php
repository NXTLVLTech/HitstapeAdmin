...
var imageChange=0;
	...
	firebase.initializeApp(config);
		var playersRef = firebase.database().ref().child('discover/musicvideos/'+videoId);
		playersRef.on("value", function(snapshot) {
		
			   text=snapshot.val().text;
			   titleDb=snapshot.val().title;
			   youtubeUrl=snapshot.val().youtubeUrl;
		    ...
		    document.getElementById("youtubeurl").setAttribute('value',youtubeUrl);
...
function	getPath()
  { 
     ...
     saveref = firebase.database().ref('discover/musicvideos');
  ...
  console.log('title  -- '+title+'  description  '+description  +'youtubeurl  '+youtubeurl);
const file = $('#wizard-picture').get(0).files[0];
//const videofile = $('#wizard-video').get(0).files[0];
  title=title.trim();
  ...
  {
  alert('Please enter description of video');
  }
  else if(youtubeurl.length==0)
  {
	  alert('Please enter youtube link of video'); 
  }
else{
	titleDb=title;
	text=description;
			    ...
			    const task = ref.child(name1).put(file, metadata);
				  task.then((snapshot) =>{
					  imageUrl=snapshot.downloadURL;
					   ...
					   // if(videoChange==1)
					  // {
						  // const videoName = n + '-' + videofile.name;
                          // const videoMetadata = { contentType: videofile.type };  
						  	    // const videoTask = ref.child(videoName).put(videofile, videoMetadata);
						  	 // videoTask.then((snapshotvideo) => {
								 // videoUrl=snapshotvideo.downloadURL;
								 // console.log('only video update');
								  // updateData();
						  // console.log('snapshotvideo     '+snapshotvideo.downloadURL);
						  // });
					  // }else{
...

{
  ...
  window.open("../videoslist.php", '_top');
    ...
    <title>Hitstape-Upload Video</title>
                            ...
                            <div class="card wizard-card" data-color="rose" id="wizardProfile">
                                <form action="" method="">
                                    ...
                                    <div style="background-color: rgb(254,160,28);">
                                        <ul>
                                            <li>
                                                <a href="#about" data-toggle="tab">Video</a>
                                            </li>
                                                    ...
                                                    <div class="input-group">
													 ...
													 <div class="input-group">
                                                        ...
                                                        <div class="form-group label-floating">
                                                            ...
                                                            <input name="youtubeurl" value= Youtube Url  id ="youtubeurl" type="text" class="form-control">
                                                        </div>
                                                    </div>
											<!--		
													<div class="pull-right">
                                           
                                            <input type="file" onchange="onVideoChange()" id="wizard-video" accept="video/*" class='btn btn-finish btn-fill btn-orange btn-wd' name='finish' value='Choose Song' />
                                        </div> -->
                                                </div>
