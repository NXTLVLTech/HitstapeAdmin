...
$increment=0;
var playersRef = firebase.database().ref().child("albums").orderByChild('timestamp');
playersRef.on("child_added", snapshot=> {
	$increment++;
	$albumID=snapshot.key;			
	$albumName=(snapshot.child("albumName").val()!=null ? snapshot.child("albumName").val(): '' );	
	$albumArtist=(snapshot.child("albumArtist").val()!=null ? snapshot.child("albumArtist").val(): '' );	
	$mixtapeType=(snapshot.child("mixtapeType").val()!=null ? snapshot.child("mixtapeType").val(): '' );	
	$numberOfPlays=(snapshot.child("numberOfPlays").val()!=null ? snapshot.child("numberOfPlays").val(): '' );	
	$artwork=(snapshot.child("artwork").val()!=null ? snapshot.child("artwork").val(): '' );	
    $("#table_body").append("<tr  id ="+$albumID+">"+
								"<td ><b>"+$increment+"</b></td>"+
								"<td><b>"+$albumName+"</b></td>"+
								"<td><b>"+$albumArtist+"</b></td>"+
								"<td><b>"+$mixtapeType+"</b></td>"+
								"<td><b>"+$numberOfPlays+"</b></td>"+
								"<td ><b>"+$artwork+"</b></td>"+												   
								"<td class='td-actions text-right'><button onclick='getRowId(this)' type='submit' class='button'>Songs</button></td>"+
								"<td class='td-actions text-right'>"+
								'<button onclick="editAlbum(this)"   type="button" rel="tooltip" class="btn btn-simple btn-dangers btn-icon"  style="color:#fea01c";>'+
                                                            '<i class="material-icons">edit</i></button>'+
									"<button onclick='getDeleteAlbum(this)' type='button' rel='tooltip' class='btn btn-simple btn-dangers btn-icon' style='color:#fea01c';><i class='material-icons'>delete</i></button></td></tr>");                    
...
function submitCall(albumID){
		$_SESSION['albumID'] =albumID.id;
 ...
 var id =row.id;
  ...
  window.open("../updateAlbum.php", '_top');
 }
...
function getDeleteAlbum(element){
	var index=element.parentNode.parentNode.rowIndex;
	var table = document.getElementById("table_body");
	var row = table.rows[index-1];
	var id =row.id;
    swal({title: 'Are you sure?',
			...
			var playersRef = firebase.database().ref("albums/");
            ...
            <nav class="navbar navbar-transparent navbar-absolute">
                <div class="container-fluid">
                    ...
                    <div class="navbar-header">
                        ...
                        <a class="navbar-brand" href="#"> Hits Tape </a>
                    </div>
                </div>
            </nav>
            <div class="content">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card">
								...
								<a href="../uploadNewAlbum.php">
									<div _ngcontent-c1=""  class="card-header card-header-icon" data-background-color="orange" style="float: right;">
										<i _ngcontent-c1="" class="material-icons">add</i>
									</div>
								</a>
