...
function getUserToken(){
	...
	if(title.length==0){
		...
		playersRef.once("value", function(snapshot) {
			totalItems=	snapshot.numChildren();
			 console.log("There are "+snapshot.numChildren()+" messages");	
			snapshot.forEach(function(child) {
				itemsProcessed++;
				console.log(child.key+": "+child.val().userToken);
				sendToken(child.val().userToken);
				
			});
			console.log('hello '+itemsProcessed);
			console.log('intjh '+intjh);
	...
	if(token!=null && token !=""){
	 ...
	 intjh++;
		$.ajax({
		  type: 'POST',
		  dataType: 'json',
		  url: 'http://hitstapeadmin.com/noti.php',
		  data: {'token': token,
		  'title':$("#title").val(),
		  'body':$("#text").val()}, 
		}) .done(function(data){
...
return false;;
    ...
    <title>Hitstape-Push Notification</title>
                ...
                <div class="container-fluid">
                    <div class="row">
                        ...
                        <div class="col-md-6" style="align:center">
                            <div class="card">
                                <form onsubmit="return getUserToken()">
