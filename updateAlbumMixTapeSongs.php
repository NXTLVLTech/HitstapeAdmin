...
$increment=0;
	...
	var playersRef = firebase.database().ref().child('discover/mixtape/'+albumid+'/songs/'+songId);
		playersRef.on("value", function(snapshot) {
		
			   songartist=snapshot.val().songArtist;
			   songlenth=snapshot.val().length;
			   songname=snapshot.val().songName;
			   imageUrl=snapshot.val().imageUrl;
			   songurl=snapshot.val().songURL;
			    songID=snapshot.val().songID;
			     timestamp=snapshot.val().timestamp;
			    //  numberOfPlays=snapshot.val().numberOfPlays;
			  
			  var image = document.getElementById("wizardPicturePreview");
			  image.src = imageUrl;
			var sng=  document.getElementById("wizard-song");
			sng.src=songurl
			 
			console.log('artist  '+songartist);	
			console.log('songlenth  '+songlenth);	
			console.log('songname  '+songname);	
			console.log('songurl  '+songurl);	
			console.log('imageUrl  '+imageUrl);	
		    document.getElementById("title").setAttribute('value',songname);
		    document.getElementById("songlength").setAttribute('value',songlenth);
		    document.getElementById("artistname").setAttribute('value',songartist);
...
function getPath(){ 
     saveref = firebase.database().ref('discover/mixtape/'+albumid+'/songs');
	var ref = firebase.storage().ref().child('albumAndSong-pics');
    var songref = firebase.storage().ref().child('mp3');
	 var title=$('#title').val();
  var artistname=$('#artistname').val();
  var songlength=$('#songlength').val();
  
  //console.log('title  -- '+title+'  artistname  '+artistname  +'songlength  '+songlength);
const file = $('#wizard-picture').get(0).files[0];
const songfile = $('#wizard-song').get(0).files[0];
  ...
  if(title.length==0)
  {
  alert('Please enter title of song');
  }
  else if(songlength.length==0)
  {
  alert('Please enter length of song');
  }
 
  else if(artistname.length==0)
  {
	  alert('Please enter name of artist');
  }
  
  else{
	  songartist=artistname;
	songname=  title;
	songlenth=songlength;
...
var n = d.getTime();
	  ...
	  loader.style.display="block";
	   ...
	   console.log('imageurl jjj ');
	console.log('imageChange '+imageChange);
	console.log('songChange '+songChange);
	  if(imageChange==1)
	  {
			  ...
			  const metadata = { contentType: file.type };
			    const task = ref.child(name1).put(file, metadata);
				  task.then((snapshot) =>{
					  ...
					  if(songChange==1)
					  {  
						  const songname = n + '-' + songfile.name;
                          const songmetadata = { contentType: songfile.type };  
				    const songtask = ref.child(songname).put(songfile, songmetadata);
						  	 songtask.then((snapshotsong) => {
								 songurl=snapshotsong.downloadURL;
								  updateData();
						  console.log('url     '+snapshotsong.downloadURL);
						  console.log('image  song update');
						  });
					  }else{
...

{
	...
	console.log('songname'+songname);
	console.log('artistname'+songartist);
	console.log('songlength'+songlenth);
  ...
  window.open("../albumsongs.php", '_top');
...
<title>Hitstape-Upload MixTape Song</title>
                ...
                <div class="user">                
					...
					<div >
                        <ul class="nav">
                            ...
                            <li style="background-color: rgb(254,160,28);"><a href="../albums.php"><span class="sidebar-normal"> Mix Tape</span></a></li>
							<li><a href="../albumlist.php"><span class="sidebar-normal">Albums</span></a></li>
                            ...
                            <div class="card wizard-card" data-color="rose" id="wizardProfile">
                                <form action="" method="">
                                    ...
                                    <div style="background-color: rgb(254,160,28);">
                                        <ul>
                                            <li>
                                                <a href="#about" data-toggle="tab">Album Songs</a>
                                            </li>
                                                ...
                                                <div class="col-sm-6">
                                                    <div class="input-group">
                                                        ...
                                                        <div class="form-group label-floating">
                                                            <label class="control-label">Song Name
                                                                <small>(required)</small>
                                                            </label>
                                                            <input  value= songname name="title"  id ="title" type="text" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="input-group">
                                                        <span class="input-group-addon">
                                                        ...
                                                        <div class="form-group label-floating">
                                                            ...
                                                            <input name="artistname"  value= ArtistName id ="artistname" type="text" class="form-control">
                                                        </div>
                                                    </div>
													 ...
													 <div class="input-group">
                                                        ...
                                                        <div class="form-group label-floating">
                                                            <label class="control-label">Songlength
                                                                <small>(required)</small>
                                                            </label>
                                                            <input name="songlength" value= Song Length id ="songlength" type="text" class="form-control">
                                                        </div>
														
														    <div class="pull-right">
                                           
                                            <input type="file"  id="wizard-song" onchange="onSongChange()" accept="audio/*" class='btn btn-finish btn-fill btn-orange btn-wd' name='finish' value='Choose Song' />
                                        </div>
                                                    </div>
                                                </div>
